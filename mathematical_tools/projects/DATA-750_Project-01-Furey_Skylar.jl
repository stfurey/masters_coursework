...
begin
	...
	weather_data = CSV.read(download(url), DataFrame)
	first(weather_data,6)
end

# ╔═╡ c34867ab-daeb-4565-8bce-7c199c00c091
begin
	using Plots

	high_temps = coalesce.(weather_data.High, NaN)
	
	plot(high_temps, title="High Temperatures in Chapel Hill, NC (2023)", xlabel="Day of the Year", ylabel="Temperature (°F)", legend = false)
end

# ╔═╡ 6b6f38d2-562f-4e0d-a4e9-4e7aabf1ccbd
begin
		using DSP

		moving_avg = conv(high_temps, fill(1/5, 5))

		moving_avg = moving_avg[3:end-2]

		plot(high_temps, title="5-day Moving Average of High Temperatures", xlabel="Day of the Year", ylabel="Temperature (F)", legend=true, label="Actual")
		
		plot!(moving_avg, label="5-day Moving Average", linewidth = 2)
	
		vline!([220], label = "Birthday", color=:red, linestyle=:dash)
end

# ╔═╡ cfdbdcc5-7d76-4e93-af05-5a748e9c94f7
begin
	using Loess

	x = collect(1:length(high_temps))
	valid_indices = findall(!isnan, high_temps)
	x_valid = x[valid_indices]
	high_temps_valid = high_temps[valid_indices]
	
	# span controls the smoothing
	loess_model = loess(x_valid, high_temps_valid, span=0.05)  
	
	loess_smoothed = predict(loess_model, x)
	
	plot(x, high_temps, title="LOESS Smoothing of High Temperatures", xlabel="Day of the Year", ylabel="Temperature (F)", label="Actual")
	
	plot!(x,loess_smoothed, label="LOESS Smoothed", linewidth = 2)

	vline!([220], label = "Birthday", color=:red, linestyle=:dash)
end

# ╔═╡ be4abcb4-62a5-4c82-9faf-ce520ad64cff
begin
	using Dates
	
	birthday = Date(2023, 8, 8)
	
	birthday = dayofyear(birthday)
	
	actual_value = high_temps[birthday]
	
	moving_avg_value = round(moving_avg[birthday])
	
	loess_value = round(loess_smoothed[birthday], digits=1)
	
	# Print the results
	println("Actual recorded value on birthday: $actual_value (F)")
	println("5-day moving average on birthday: $moving_avg_value (F)")
	println("LOESS estimate on birthday: $loess_value (F)")
end

# ╔═╡ e2e64637-00e5-41ba-87f9-3fe230b6665b
begin
	using LinearAlgebra
	A = rand(30, 30)
	b = rand(30)
...
The loess model with a span on 0.05 is a better estimate for the temperature on my birthday in 2023 compared to the 5 day moving average.
"

# ╔═╡ 72fa803a-1b7c-45bc-b971-f94775bdd133
...
# ╔═╡ 94ba1de1-3157-4c8e-8c9a-95bf225611e4
x_backslash = A \ b

# ╔═╡ 2f820f1e-6231-4c84-949f-1b6e61374fcd
residual_norm_backslash = round(norm(A * x_backslash - b))

# ╔═╡ 3c2a36e6-3021-4579-a384-201a9a40c656
x_inv = inv(A) * b


# ╔═╡ f7954f61-8350-44fd-9ce4-27f8bcadee31
residual_norm_inv = round(norm(A * x_inv - b))

# ╔═╡ c909a2ef-6e7f-439a-b7b2-76bc2367da06
x_pinv = pinv(A) * b

# ╔═╡ 3f5c74b9-5123-4f2e-9c72-63252a40f82a
residual_norm_pinv = round(norm(A * x_pinv - b))

# ╔═╡ 68038980-847d-4fb2-89df-22fe87f8879f
begin
	Q, R = qr(A)
	x_qr = R \ (Q' * b)
end

# ╔═╡ 575dcb77-8fc9-4f9f-88c3-816503f79ded
residual_norm_qr = round(norm(A * x_qr - b))

# ╔═╡ 83c8cd81-23a6-4cff-b744-d910ceca8c0d
md"
Commentary on this exercise can go here

In this example, since A is invertible pinv works. All of the methods were able to quickly solve the equation Ax = b. For simplicity sake, I like the backslash method the most.
...
Perform an **inverse discrete Fourier transform** on $y.$ Print out the _absolute values_ of some of the complex coefficients $f_{k}$ to verify that, despite the random
...
# ╔═╡ 1669ae51-3167-4831-bad3-7860436757a6
begin
	...
	y = 60 .+ 20*sin.(2π*k./(24*365)) .+ 15*sin.(2π*k./24) .+ 5*(r .- 0.5)
...
begin
	...
	abs_frequencies = abs.(f)
end

# ╔═╡ 35c56e9b-ada9-4d10-b732-767ed784d655
findall(x -> x > 5, abs_frequencies)

# ╔═╡ 15efdb39-ba7c-434f-9d75-5c2684ab94d0
begin
	
	plot(abs_frequencies, title= "Cyclical Weather Station Temperatures", xlabel = "Hours", ylabel = "Temperature Signal", legend = false)
...
As expected the peaks are at 0, N/(24*365), N/24, N-N/24 and N. 

The random component didn't affect the cycle much as it highly depended on the daily cycle and yearly cycle.
...
manifest_format = "2.0"
project_hash = "6ec2af439b9ad3d16d5e9f82d517f2e13746fe23"
